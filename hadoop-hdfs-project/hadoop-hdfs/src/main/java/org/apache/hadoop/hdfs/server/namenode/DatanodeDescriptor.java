...
public class DatanodeDescriptor extends DatanodeInfo implements Serializable{
  ...
  private static class BlockQueue implements Serializable{
    ...
    synchronized List<BlockTargetPair> poll(int numBlocks) {
      ...
      //NameNode.LOG.info("before dequeue,"+blockq.size());
      List<BlockTargetPair> results = new ArrayList<BlockTargetPair>();
      for(; !blockq.isEmpty() && numBlocks > 0; numBlocks--) {
        results.add(blockq.poll());
      }
      //NameNode.LOG.info("after dequeue,"+blockq.size());
      return results;
    }
  }
  ...
  public Map<String, Integer> getDataset2NumBlocks() {
  return dataset2NumBlocks;
  }

  public void setDataset2NumBlocks(Map<String, Integer> dataset2NumBlocks) {
  this.dataset2NumBlocks = dataset2NumBlocks;
  }

  public AtomicInteger getNumBlocksWithDatasetID() {
  return numBlocksWithDatasetID;
  }

  public void setNumBlocksWithDatasetID(AtomicInteger numBlocksWithDatasetID) {
  this.numBlocksWithDatasetID = numBlocksWithDatasetID;
  }
  public int increaseNumBlocksWithDatasetID(int delta) {
    return numBlocksWithDatasetID.addAndGet(delta);
  }
  ...
  public synchronized int numBlocks(String datasetID) {
    if (dataset2NumBlocks.get(datasetID)==null) {
      dataset2NumBlocks.put(datasetID, 0);
      return 0;
    }
    return dataset2NumBlocks.get(datasetID);
  }

public synchronized void  resetLoad() {
    System.out.println("resetLoad,"+getHostName());
    this.load = new AtomicInteger(0);
  }

  public synchronized int increaseLoad(int delta) {
    //StringUtility.debugSpace("increaseLoad");
    //System.out.println("increasedLoad,"+getHostName());
    int newLoad = load.addAndGet(delta);
    //System.out.println("newLoadva, "+newLoad);
    System.out.println("increaseLoad, "+getHostName()+", "+load.get());
    return newLoad;
  }
  public double getHitRatio(String workingDataset) {
    //System.out.println("numBlocks, "+numBlocks());
    //System.out.println("load, "+load.get());
    return (double) (getLoad()*1.0/numBlocks(workingDataset));
  }
